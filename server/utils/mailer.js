...
async function sendContactEmail({ name, email, phone, subject, message }) {
  ...
  await resend.emails.send({
    from: "Omniform Pharma <inquiry@omniformpharma.com>",
    to: "inquiry@omniformpharma.com",
    replyTo: email,
    subject: `Website Enquiry${cleanSubject ? ` - ${cleanSubject}` : ""}`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827;">
        ...
        <table style="border-collapse: collapse; margin-bottom: 16px;">
          <tr>
            <td style="padding: 4px 12px 4px 0; font-weight: 700;">Name</td>
            <td style="padding: 4px 0;">${name}</td>
          </tr>
          <tr>
            <td style="padding: 4px 12px 4px 0; font-weight: 700;">Email</td>
            <td style="padding: 4px 0;"><a href="mailto:${email}">${email}</a></td>
          </tr>
          <tr>
            <td style="padding: 4px 12px 4px 0; font-weight: 700;">Phone</td>
            <td style="padding: 4px 0;">${cleanPhone || "-"}</td>
          </tr>
          <tr>
            <td style="padding: 4px 12px 4px 0; font-weight: 700;">Subject</td>
            <td style="padding: 4px 0;">${cleanSubject || "-"}</td>
          </tr>
        </table>
        <div style="padding: 12px 14px; background: #F3F4F6; border-radius: 8px; white-space: pre-wrap;">${message}</div>
      </div>
    `,
  });
}
